...
- `packages/db`: shared Drizzle/Bun SQLite client and schema, exported as `@sf/db`; migrations and seeding belong here.
...
- Tasks come only from each project's `moon.yml`; package scripts are not automatically moon tasks. API exposes only `api:run`; DB exposes generation, migration, push, seed, studio, lint, and typecheck targets.
...
- Always set `DATABASE_PATH` explicitly when running the API, migrations, or seed, and point all three at the same file. Defaults currently disagree, `packages/db/src/client.ts` has no fallback, and relative paths resolve from the command's working directory.
...
- Seed with `DATABASE_PATH=/absolute/path/to.db moon run db:seed`. No external database service is required.
...
- DB verification: `moon run db:lint`, then `moon run db:typecheck`; DB operations are `db:generate`, `db:migrate`, `db:push`, `db:seed`, and `db:studio`.
...
- Flutter run/release APK: `moon run sf_consumer:run` / `moon run sf_consumer:build-apk`.
